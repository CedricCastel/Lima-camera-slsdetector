...
SPECDIR=calibrationWizards
TMPFILE=gitInfoCalWizTmp.h
INCLFILE=gitInfoCalWiz.h
WD=$PWD
...
RDATE1="find . -type f -exec stat --format '%Y :%y %n' '{}' \; | sort -nr | cut -d: -f2- | egrep -v 'gitInfo|build|.git|updateGitVersion|enCalDict.cpp' | head -n 1"
RDATE=`eval $RDATE1`
